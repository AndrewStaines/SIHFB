...
function Home() {
	...
	const handleOnSubmit = async (e) => {
		...
		'http://localhost:2000/register', {
			method: "post",
			body: JSON.stringify({ abstract, desc , domain}),
			headers: {
...
<input type="text"value={domain} className='border-solid border-2 border-black-500' onChange={(e) => setdom(e.target.value)} placeholder='Domain of the Paper' id='0'></input><br/><br/>
						<div className="mb-2">
  <label for="exampleFormControlTextarea1" className="form-label">Description</label><br/>
  <textarea className="form-control" id="exampleFormControlTextarea1" rows="3" value={desc} onChange={(e) => setdesc(e.target.value)}></textarea>
</div>
